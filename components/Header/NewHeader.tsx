...
const NewHeader: React.FC<Props> = ({ isHome, headerMenu, logoSettings }) => {
  ...
  return (
    <div
      className={`
        bg-templateBackground relative z-10 transition-all ease-in-out duration-300 text-templateText space-y-2 pt-3 pb-1 w-full 
      `}
    >
      <div className="px-4 md:px-8  lg:px-10 flex items-center justify-between">
        <div className="lg:w-full flex items-center justify-start">
          <SearchBarComp variation={2} />
          <MobileNavigation variation={2} headerMenu={headerMenu} />
        </div>
        ...
        <div className="lg:w-full flex items-center justify-end gap-2.5 lg:gap-6">
          {validated && (
            <div className="hidden lg:block">
              <HeaderAccountComp variation={2} />
            </div>
          )}
          <CartDrawer variation={2} />
        </div>
      </div>

      <div className="lg:hidden">
        <MobileSearchComp variation={2} />
      </div>
          ...
          {headerMenu.map((item, index) => (
            ...
            >
              <Link
                ...
                className={`
                  py-1 px-2 relative font-medium text-[#242424] hover-class-2 text-[15px] flex items-center gap-0.5
              ...
              <AnimatePresence>
                {activeDropdownMenu?.name === item.name && item.dropdown && (
                  <motion.div
                    ...
                    className="absolute min-w-[300px] overflow-hidden top-[34.5px] left-0 bg-white shadow z-40"
                    onMouseLeave={clearMenus}
                  >
                    <div
